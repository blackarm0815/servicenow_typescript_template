// eslint-disable-next-line
const networkAdapterTester = (cmdbCiNetworkAdapterSysIdArray: Array<string>) => {
  //
  const report: Record<string, Array<string>> = {};
  return report;
};
// end of script include
// remove everything below this before putting it in the script include
const cmdbCiNetworkAdapterSysIdArray = [
  'bc22df4adb1ec70cab79f7d41d9619f6',
  'b817db4edb168bc010b6f1561d961914',
  'f4738c21dbb1c7442b56541adc96196a',
  'b1c34461dbb1c7442b56541adc96198f',
  'efd3cc61dbb1c7442b56541adc961978',
  'bdba2b74db271788259e5898dc9619a4',
  '3abaa3f4db271788259e5898dc9619ab',
  '3bba63f4db271788259e5898dc961971',
  '30cae3f4db271788259e5898dc961926',
  '0aca67f4db271788259e5898dc961979',
];
const result = networkAdapterTester(cmdbCiNetworkAdapterSysIdArray);
// @ts-ignore
gs.print(result);
